...
half 		  unity_sampleShadowmapNGSS( float4 coord );		// sample shadowmap SM2.0+
...
/**
 *	DEfault Unity Soft Shadow (SM 3.0)
 */
fixed4 frag_hard(v2f i) : SV_Target
{
#if UNITY_VERSION >= 560
	UNITY_SETUP_INSTANCE_ID(i); // required for sampling the correct slice of the shadow map render texture array
#endif
...
/**
 *	NGSS SHADOWS (SM3.0 recommended)
 */
half unity_sampleShadowmapNGSS( float4 coord, float2 receiverPlaneDepthBias )
{
	...
	for(int i = 0; i < 25; ++i)
	{
		...
		result += UNITY_SAMPLE_SHADOW(_ShadowMapTexture, combineShadowcoordComponents(coord.xy, rotatedOffset, coord.z, receiverPlaneDepthBias)).x;//
		//result += UNITY_SAMPLE_SHADOW(_ShadowMapTexture, float4( coord.xy + rotatedOffset, coord.zw)).x;
	}
	half shadow = dot(result, 0.05);//0.04
	return shadow;
}
...
fixed4 frag_pcf5x5 (v2f i) : SV_Target
{
	...
	half shadow = unity_sampleShadowmapNGSS(coord, receiverPlaneDepthBias);
	...
	UNITY_BRANCH
		if (alpha > 1 - UNITY_CASCADE_BLEND_DISTANCE)
		{
			...
			half shadowNextCascade = unity_sampleShadowmapNGSS(coord, receiverPlaneDepthBias);
...
SubShader {
	...
	Pass {
		...
		CGPROGRAM
		#pragma vertex vert
		//If you ever what to go back to hard shadows uncomment this line and comment the rest of #pragma lines
